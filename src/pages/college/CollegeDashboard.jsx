...
function CollegeDashboard() {
  ...
  useEffect(() => { api.get("/colleges/analytics").then(({ data }) => setAnalytics(data)).catch((err) => setError(err.response?.data?.message || "Unable to load college analytics.")); }, []);
  ...
  const roles = analytics.careerRoles || [];
  return <div className="dashboard-layout"><Sidebar type="college" /><main className="main-content">
    ...
    <div className="stats-grid">{[['Total Students', total], ['Students Placed', placed], ['Placement Rate', `${analytics.placementRate || 0}%`], ['Companies Engaged', analytics.companiesEngaged || 0]].map(([label,value]) => <div className="stat-card" key={label}><span className="stat-label">{label}</span><div className="stat-value">{value}</div><p className="stat-description">Live data from your college</p></div>)}</div>
    <div className="dashboard-grid"><div className="card"><div className="section-title"><h2>Student Skill Distribution</h2><span>{total} Students</span></div><div className="skill-list">{topSkills.length ? topSkills.map(({skill,count}) => { const percent = total ? Math.round((count/total)*100) : 0; return <div className="skill-row" key={skill}><div className="skill-row-header"><span>{skill}</span><strong>{percent}%</strong></div><div className="progress-bar"><div className="progress-fill" style={{width:`${percent}%`}} /></div></div>; }) : <div className="no-results"><h3>No Skill Data</h3><p>Student skills will appear here when profiles are completed.</p></div>}</div></div>
      <div className="card"><div className="section-title"><h2>Career Roles</h2><span>{roles.length} Roles</span></div><div className="demand-list">{roles.length ? roles.slice(0,6).map(({role,count}) => <div className="demand-item" key={role}><div><strong>{role}</strong><p>{count} students</p></div><span>{total ? Math.round((count/total)*100) : 0}%</span></div>) : <div className="no-results"><h3>No Career Data</h3><p>Student career preferences will appear here.</p></div>}</div></div></div>
